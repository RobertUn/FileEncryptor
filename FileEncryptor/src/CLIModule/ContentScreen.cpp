...
ContentScreen::ContentScreen() : dm(MAIN_FILE) {
    list_content = { {0, "Назад"} };
    nextScreen = ScreenType::CONTENT;
}

std::string ContentScreen::getURL(int item_index) {
    auto* item = dm.getItem(item_index);
    return item ? item->url : "";
}

bool ContentScreen::copyToClipboard(std::string_view text) {
...
void ContentScreen::handleInput(int input) {
    if (input == 0) {
