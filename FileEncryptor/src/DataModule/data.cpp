...
private:
    ...
    std::string convertEncoding(const std::string& input,
        ...
        if (strcmp(from_encoding, "UTF-8") == 0) from_cp = CP_UTF8;
        else if (strcmp(from_encoding, "CP1251") == 0) from_cp = 1251;
        else return input;

        if (strcmp(to_encoding, "UTF-8") == 0) to_cp = CP_UTF8;
        else if (strcmp(to_encoding, "CP1251") == 0) to_cp = 1251;
        else return input;
...
public:
    DataManager(const std::string& file_name) : file_name(file_name) {}
    ...
    void sortItemsByName() {
        std::sort(items.begin(), items.end(),
            [](const Item& a, const Item& b) { return less(a.name, b.name); });
    }
    ...
    // Добавить элемент. Плохая реализация нужно проверять уникальность индекса
